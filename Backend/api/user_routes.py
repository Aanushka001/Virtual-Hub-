...
@user_ns.route('/profile/<string:user_id>')
...
@user_ns.route('/recommendations')
class UserRecommendationsResource(Resource):
    @require_auth
    def get(self):
        ...
        try:
            ...
            group_recommendations = matching_service.get_recommended_groups(user_profile)
            mentor_recommendations = matching_service.get_recommended_mentors(user_profile)
            resource_recommendations = matching_service.get_recommended_resources(user_profile)
            opportunity_recommendations = matching_service.get_recommended_opportunities(user_profile)
    ...
    def get(self):
        ...
        try:
            ...
            stats = {
                ...
                'mentorships': len([c for c in user_connections if c.get('status') == 'accepted']),
                'resources': len(user_resources),
                'points': g.current_user.get('points', 0),
                'level': g.current_user.get('level', 1),
                'badges': len(g.current_user.get('badges', []))
            }
